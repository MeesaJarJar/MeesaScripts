...
# -------------------------------------------------------------#
# START CONFIG ------------------------------------------------#
furnitureDyeTubSerial = 0x43A9D040
dyes_serial = 0x415BFB62
...
def updateGump():
    ...
    Gumps.AddLabel(gd, 5, 59, selectedHue-1, "Hue:")
    #print("XSELECT:", selectedHue)
    Gumps.AddTextEntry(gd, 30, 60, 50, 25, selectedHue-1, 0, str(selectedHue))  # Ensure the default value is set to selectedHue
    ...
    Gumps.AddButton(gd, 105, 77, 4005, 4006, 2, 1, 0)    
    Gumps.AddLabel(gd, -5, 77, 2058, "Copy Hue")
    ...
    Gumps.AddLabel(gd, 15, -10, 1152, 'Meesa Jar Jar`s!')
    Gumps.AddLabel(gd, 30, 3, 1152, 'ItemHueDyer')
...
def dyeFurniture(color, itemToDye):
    changecolor(color)
    Misc.Pause(700)
    Items.UseItem(furnitureDyeTubSerial)
    Misc.Pause(700)
...
def changecolor(color):
    ...
    dyeTub = Items.FindBySerial(furnitureDyeTubSerial)
    if dyeTub:
